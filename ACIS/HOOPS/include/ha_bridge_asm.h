...
/*******************************************************************/
#ifndef __HA_BRIDGE_ASM_H_
#define __HA_BRIDGE_ASM_H_
#include "hc.h" // for use of HC_KEY
#include "dcl_hoops.h"
#include "ha_util.h"
/**
 * @file ha_bridge_asm.h
 * @CAA2Level L1
 * @CAA2Usage U1
 *! \addtogroup HABRIDGEAPI
 *  \brief Declared at <ha_bridge_asm.h>
 *  @{
 */
class ASM_ASSEMBLY;
class ASM_MODEL_REF;
 ...
 */
DECL_HOOPS void
HA_Delete_Model_Ref(ASM_MODEL_REF* mref);
 ...
 */
DECL_HOOPS void
HA_Delete_Model(asm_model* model);
 ...
 */
DECL_HOOPS void
HA_Delete_Model_Geometry(asm_model* model);
 ...
 * @param count
 * Maximum number of keys that should be returned, see @href HA_Compute_Model_Key_Count.
 * @param keys
 ...
 */
DECL_HOOPS unsigned long
HA_Compute_Model_Keys(asm_model* model, unsigned long count, HC_KEY* keys);

/**
 * Returns the number of HOOPS segments in the HOOPS database that represent the given ACIS assembly model.
 * <br><br>
 * <b>Role:</b> This allows allocating an appropriate sized keys array for a subsequent <tt>HA_Compute_Model_Keys</tt> call.
 * <br><br>
 * @param model
 * Pointer to an ACIS assembly model.
 * <br><br>
 ...
 */
DECL_HOOPS unsigned long
HA_Compute_Model_Key_Count(asm_model* model);
 ...
 */
DECL_HOOPS asm_model*
HA_Compute_Model_Pointer (HC_KEY key);
 ...
 */
DECL_HOOPS logical
HA_Associate_Key_To_Model(asm_model* model, HC_KEY key);
 ...
 */
DECL_HOOPS logical
HA_Disassociate_Key_From_Model(asm_model* model, HC_KEY key);

/**
 * Returns the number of HOOPS segments in the HOOPS database that represent the given ACIS assembly component.
 * <br><br>
 * <b>Role:</b> This allows allocating an appropriate sized keys array for a subsequent <tt>HA_Compute_Component_Keys</tt> call.
 * <br><br>
 * @param comp
 ...
 */
DECL_HOOPS unsigned long
HA_Compute_Component_Key_Count(component_handle* comp);
 ...
 * Pointer to an ACIS assembly component handle.
 * @param count
 * Maximum number of keys that should be returned, see @href HA_Compute_Component_Key_Count.
 * @param keys
 * Array of associated HOOPS keys. Returned to user.
 * <br><br>
 * @return
 ...
 */
DECL_HOOPS unsigned long
HA_Compute_Component_Keys(component_handle* comp, unsigned long count, HC_KEY* keys);
 ...
 * The requested ACIS pointer.
 */
DECL_HOOPS component_handle*
HA_Compute_Component_Pointer (HC_KEY key);
 ...
 * The requested ACIS pointer.
 */
DECL_HOOPS component_entity_handle*
HA_Compute_Component_Entity_Pointer(unsigned long count, HC_KEY* keys);
 ...
 */
DECL_HOOPS component_entity_handle*
HA_Compute_Component_Entity_Pointer(HC_KEY comp_key, HC_KEY ent_key);
 ...
 */
DECL_HOOPS logical
HA_Compute_Component_Entity_Keys(component_entity_handle* comp_ent, HC_KEY& comp_key, HC_KEY& ent_key);
 ...
 */
DECL_HOOPS logical
HA_Associate_Key_To_Component(component_handle* comp, HC_KEY key);
 ...
 */
DECL_HOOPS logical
HA_Disassociate_Key_From_Component(component_handle* comp, HC_KEY key);
 ...
 */
DECL_HOOPS void
HA_Highlight_Component_Entity(component_entity_handle* ce, logical on, const rgb_color& color);
 ...
 * <i><b>Note:</b> <tt>HA_Render_Model</tt> will use default pattern set by "segment pattern" option
