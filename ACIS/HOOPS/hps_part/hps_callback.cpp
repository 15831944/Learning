...
// Modifications
//
// jce (06/07/06) : Moved the body of HPS_EntityCallback::execute from
//                  hps_callback.h to this new source file
...
void HPS_EntityCallback::execute( entity_event_type ev_type, ENTITY *ent )
{
	if ( !ent )
		return;
	...
	if ( !IS_LIGHT( ent ) && !is_toplevel( ent ) )
	{
		...
		if ( Is_In_Top_Map( ent ) )
		{
			...
			{
				HPS_Delete_Entity_Geometry( ent );
	...
	case pm_Part_Delete_Delete_Entity:
		...
		// database. Code for HPS_Delete_Entity_Geometry has been changed to do that
		HPS_Delete_Entity_Geometry( ent );
		break;
...
void HPS_History_Callbacks::Before_Roll_Bulletin_Board( BULLETIN_BOARD*, logical discard )
{
...
void HPS_History_Callbacks::After_Roll_Bulletin_Board( BULLETIN_BOARD*, logical discard )
{
	...
	HPS_Delete_Entity_Geometry( delete_ents_list );

	create_ents_list.add( change_ents_list );
	HPS_Render_Entities( create_ents_list );
