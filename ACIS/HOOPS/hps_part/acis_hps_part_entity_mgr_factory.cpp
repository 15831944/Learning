...
#include "acis_hps_part_entity_mgr_factory.hxx"
...
acis_hps_part_entity_mgr_factory::acis_hps_part_entity_mgr_factory( const VOID_LIST& seed_parts ) :
...
asm_model_entity_mgr* acis_hps_part_entity_mgr_factory::create_entity_mgr_vf( asm_model_info const & model_info,
																			  HISTORY_STREAM* share_stream )
{
	...
	if ( result.ok() && part )
	{
		...
		if ( mgr == NULL )
		{ mgr = ACIS_NEW acis_hps_part_entity_mgr( part ); }
